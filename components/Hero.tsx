...
const Hero: React.FC = () => {
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center pt-24 pb-20 overflow-hidden bg-[#F4F3EF]">
      {/* Background Grid - even subtler for maximum brightness */}
      ...
      <div className="z-10 text-center px-6 max-w-6xl mx-auto flex-1 flex flex-col justify-center">
        <div className="space-y-10 mb-20">
          ...
          <p className="max-w-3xl mx-auto text-xl md:text-3xl text-[#2F4F68]/70 font-light leading-relaxed">
            M&A deals are won in the first five minutes. We deploy human-parity intelligence to capture high-stakes deal flow <span className="text-[#2F4F68] font-semibold italic underline decoration-[#E6B65C] underline-offset-8">with institutional precision.</span>
          </p>
        ...
        {/* Hero Navigation Tabs - Clean light interface */}
          ...
          <div className="flex flex-wrap justify-center gap-px bg-[#2F4F68]/5 border border-[#2F4F68]/5 rounded-sm overflow-hidden backdrop-blur-xl shadow-lg">
            {navTabs.map((tab, idx) => (
              <button
                ...
                className="flex-1 min-w-[160px] py-8 px-6 bg-white/40 hover:bg-white transition-all duration-500 group relative border-r border-[#2F4F68]/5 last:border-r-0"
              >
                ...
                <span className="text-[11px] font-cinzel text-[#2F4F68]/50 group-hover:text-[#2F4F68] uppercase tracking-[0.25em] font-black transition-all group-hover:tracking-[0.3em]">
                  {tab.label}
                </span>
              </button>
            ))}
