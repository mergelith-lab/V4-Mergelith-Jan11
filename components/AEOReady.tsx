...
const AEOReady: React.FC = () => {
  ...
  return (
    <section id="aeo-readiness" className="scroll-mt-32">
      <div className="bg-navy text-pearl p-16 md:p-24 rounded-sm shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-96 h-96 bg-gold/5 rounded-full -mr-48 -mt-48 blur-3xl"></div>
        
        <div className="relative z-10 grid lg:grid-cols-2 gap-20">
          <div className="space-y-10">
            <div>
              <h3 className="text-[11px] uppercase tracking-[0.5em] text-gold font-black mb-4">AEO Deployment</h3>
              ...
              <div className="mb-10 p-6 bg-white/5 border-l-2 border-gold rounded-r-sm backdrop-blur-sm">
                <p className="text-pearl font-medium text-lg leading-relaxed">
              ...
              <p className="text-pearl/70 font-light text-base leading-relaxed">
            ...
            <div className="space-y-4">
              <h4 className="text-[10px] uppercase tracking-[0.4em] text-gold font-black">Optimization Checklist</h4>
              <ul className="space-y-4">
                {checklist.map((item, idx) => (
                  <li key={idx} className="flex items-start gap-4 text-sm text-pearl/60 font-light italic">
          ...
          <div className="bg-white/5 border border-white/10 p-12 rounded-sm backdrop-blur-md">
            <h4 className="text-[10px] uppercase tracking-[0.4em] text-gold font-black mb-8 text-center">Structured Data Strategy</h4>
            <div className="space-y-8">
              {schemaTypes.map((schema, idx) => (
                <div key={idx} className="flex gap-6 items-start">
                  <div className="px-3 py-1 bg-gold/10 border border-gold/20 rounded text-[10px] font-mono text-gold whitespace-nowrap">
                    {schema.type}
                  </div>
                  <p className="text-xs text-pearl/50 leading-relaxed font-light italic">
                    {schema.reason}
                  </p>
                </div>
              ))}
            ...
            <div className="mt-12 pt-8 border-t border-white/5 text-center">
              <p className="text-[10px] uppercase tracking-[0.5em] text-gold/40 font-black">Mergelith Institutional Protocol Active</p>
            </div>
          </div>
        </div>
      </div>
